...
## Phase 4: [US1] 月次予測列でKPIを編集し予測を得る (P1)
...
- [ ] T013 [US1] Add forecast columns and KPI custom cell renderer/editor at src/utils/reportGrid.tsx
- [ ] T014 [US1] Wire KPI cell edit to HyperFormula (evaluate/update deps) at src/components/AggregatePanel.tsx
- [ ] T015 [US1] Persist KPI/forecast (value/formula) on edit at src/hooks/useForecastService.ts
- [ ] T016 [US1] Disable edit on actual cells at src/utils/reportGrid.tsx
- [ ] T017 [US1] Add integration test for forecast API at tests/integration/tagging-report.test.ts

## Phase 5: [US3] 実績セルは参照専用 (P2)

- [ ] T022 [US3] Enforce read-only on actual cells in src/utils/reportGrid.tsx
- [ ] T023 [US3] Block paste/delete on actual cells in src/components/AggregatePanel.tsx

## Final Phase: Polish & Cross-Cutting

- [ ] T024 Add error banners and formula error highlighting in src/components/AggregatePanel.tsx
- [ ] T025 Add undo/redo support for KPI cells in src/components/AggregatePanel.tsx
- [ ] T026 Document quickstart updates in specs/003-speckit-specify-n/quickstart.md

## Dependencies
- US2 → US1 → US3 の順で進行（US1はタグ種別UI/非表示ロジックの前提に依存）。
...
- MVPはUS2完了後にUS1のKPIセル編集→予測再計算→保存まで。US3/Polishは後続。
